...
func OnGetTotalNewTicket(_tenant, _company, _duration int, result chan string) {
	...
	url := fmt.Sprintf("http://%s/render?target=sumSeries(stats.gauges.event.totalcount.%d.%d.total.NEWTICKET)&from=-%dd&format=json", statsDIp, _tenant, _company, _duration)
	resultChannel := make(chan string)
	go IncokeGhaphite(url, resultChannel)
...
func OnGetTotalClosedTicket(_tenant, _company, _duration int, result chan string) {
	...
	url := fmt.Sprintf("http://%s/render?target=sumSeries(stats.gauges.event.totalcount.%d.%d.total.CLOSEDTICKET)&from=-%dd&format=json", statsDIp, _tenant, _company, _duration)
	resultChannel := make(chan string)
	go IncokeGhaphite(url, resultChannel)
	var ticketInfo = <-resultChannel
	close(resultChannel)
...
func OnGetDiffClosedVsNew(_tenant, _company, _duration int, result chan string) {
	...
	url := fmt.Sprintf("http://%s/render?target=diffSeries(stats.gauges.event.totalcount.%d.%d.total.NEWTICKET,stats.gauges.event.totalcount.1.103.total.CLOSEDTICKET)&from=-%dd&format=json", statsDIp, _tenant, _company, _duration)
	resultChannel := make(chan string)
	go IncokeGhaphite(url, resultChannel)
	var ticketInfo = <-resultChannel
	close(resultChannel)
